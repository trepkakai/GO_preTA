...
This is an R Markdown file for plotting the Draft 4 version of all GO Study Figures. Most modeling is conducted in separate scripts, with outputs loaded here for plotting. 
...
# Prefix different if local ("/Volumes/turnbaughlab/qb3share/") vs Rstudioserver ("/mnt/tank/labmainshare/qb3share/")
...
dir = paste0(prefix, "ktrepka/GOStudy/1_FIGURES/Figures_bioRxiv/")
...
preta_palette <- c("preTA++" = "blue", "ΔpreTA" = "orange")
preta_surv_palette <- c("Group=OE" = "blue", "Group=KO" = "orange")
...
p <- ggplot(stat_bl, aes(x = Days_Since_Tx, y = Simpson_Diff)) + 
  geom_smooth(method = stats::loess, color = "black", fill = "orange", se = TRUE, span = 0.9, fullrange = TRUE, level = 0.68) + # level = 0.68 is standard error: https://forum.posit.co/t/geom-smooth-shading-based-on-standard-error/154449/2, https://stat.ethz.ch/pipermail/r-sig-mixed-models/2019q4/028218.html, https://www.reddit.com/r/RStudio/comments/irxcf0/help_exporting_geom_smooth_regression_line_into_a/
  theme_pubr() + 
  xlab("Days since treatment start") + 
...
## KO GSEA Early
Current approach: Filter to genes with FDR < 0.2 that are enriched early, perform overrepresentation analysis. 

Another approach gives the same result: Filter to genes with nominal p < 0.01 that are enriched early, perform overrepresentation analysis. 
```{r}
...
# GSEA - http://yulab-smu.top/biomedical-knowledge-mining-book/clusterprofiler-kegg.html
...
p <- ggplot(hm, aes(x = Sample_ID, y = Microbe, fill = RPKG)) + 
  ...
  theme(axis.text.x = element_blank(), legend.position = "top") + #, axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
 theme(legend.title.align=0.5) +
...
k_plot$GeneRatio <- apply(k_plot %>% select(GeneRatio), c(1,2), function(x) eval(parse(text = x)))
k_plot$BgRatio <- apply(k_plot %>% select(BgRatio), c(1,2), function(x) eval(parse(text = x))) # # glob <- c("ko01100", "ko01120", "ko01110", "ko01200", "ko01210", "ko01212", "ko01230", "ko01232", "ko01250", "ko01240", "ko01220") # Remove global/overview kegg pathways b/c not meaningful: https://www.genome.jp/kegg/pathway.html#global. 
glob <- c(glob, "ko03010", "ko00195", "ko00710", "ko04626", "ko04112", "ko00643", "ko05134") # remove ribosome, photosynthesis, Caulobacter
k_plot <- k_plot %>% filter(!(ID %in% glob))
...
```{r}
# Hit df
hits <- c("1.3.1.1", "1.3.1.2", "3.5.2.2", "2.4.2.3", "2.4.2.2", "3.5.4.12")
...
a_plot$GeneAnnotations <- case_when(a_plot$GeneFamily == "1.3.1.1" ~ "preTA (EC 1.3.1.1)",
                               ...
                               a_plot$GeneFamily == "2.4.2.3" ~ "upp (EC 2.4.2.3)",
...
# Figure S5 - SIC supplement
```{r}
# Make directory
figdir <- paste0(dir, "FigureS5/")
dir.create(figdir)
```
...
p <- ggplot(g, aes(x = Group, y = k)) + 
  ...
  #stat_compare_means(method = "kruskal.test", label.x = 1.3, label.y = 2.1, aes(label = paste0("italic(p) ==", after_stat(p.format))), parse = TRUE) +
...
p <- ggplot(g, aes(x = Group, y = Observed_Rarefied)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  #stat_compare_means(method = "kruskal.test", label.x = 1.6, aes(label = paste0("italic(p) ==", after_stat(p.format))), parse = TRUE) +
  stat_compare_means(comparisons = list(c("Veh", "CAP"), c("Veh", "5FU"))) +
  scale_color_manual(values = cap_palette) + 
  scale_fill_manual(values = cap_palette) + 
  theme(legend.position = "none") + 
...
Also termed "thermal hyperalgesia"
...
# Figure S6 - supplemental mechanisms of toxicity
```{r}
# Make directory
...
# Figure S7 - CAP alters microbiome mice
```{r}
# Make directory
...
p <- ggplot(pca_plot, aes(x = .data[[paste0("Dim.", axis1)]], y = .data[[paste0("Dim.", axis2)]])) +
  ...
  #geom_path(aes(group = Mouse), arrow = arrow(type = "closed", length = unit(0.02, "inches")), lineend = 'round', linewidth = 0.25, color = "grey") + 
  theme_pubr() +
...
v <- u %>% filter(FDR < fdr_cutoff)
toi <- v$Taxa
#toi <- toi[!grepl("GGB", toi)] # exclude unclassified
...
p <- p + scale_color_manual(values = conv_surv_palette) +
  ...
  theme(legend.position = "none") #+ 
  #scale_linetype_manual(values = c("Group=SPF"="dashed","Group=CONV-D"="dotted","Group=GF"="dashed") )
...
p <- p + scale_color_manual(values = avnm_surv_palette) +
  ...
  annotate("text", x = 3, y = 0.1, label = paste("italic(p) == ", formatC(signif(p_value, digits=2), digits = 2, format="fg", flag="#")), parse = TRUE) + 
  theme(legend.position = "none")
  #annotate("text", x = 11, y = .5, label = sprintf("p = %.4f", p_value))
...
# Figure S8 - validation of microbiota depletion
```{r}
# Make directory
...
p <- ggplot(ce, aes(x = Group, y = Weight)) + 
  ...
  stat_compare_means(method = "wilcox", label.x=1.25, label.y = 1400, aes(label = paste0("italic(p) ==", after_stat(p.format))), parse = TRUE) + 
  scale_color_manual(values = avnm_palette) + 
  scale_fill_manual(values = avnm_palette) + 
  theme(legend.position = "none") + 
  ylim(c(200, 1450))
...
# Figure S9 - GF model
This data (Figure S9) is all from Peter S toxicity experiment. 6-8 week old male C57BL/6 mice were treated with 1500 mg/kg once daily. 
```{r}
# Make directory
...
Common toxicities table: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4114495/
...
progression <- c("6", "14", "17", "20", "30", "47") # "progression"
...
#pts <- unique(progression) - this has opposite trend!! Becomes significant if we add "notox_validated"
...
ToxDF = data.frame(Toxicity = Toxicity, Number = Number, Percent = Number/40*100)
...
p <- ggplot(ToxDF, aes(x = reorder(Toxicity, Number), y = Number)) +
  ...
  geom_text(aes(label = Number), hjust = -0.3) + # Add text labels
...
# Exclude radiation patients
#delta <- delta %>% filter(Metadata_Radiation.x == "No")
...
#data_lr$Dose <- case_when(data_lr$ColonLengthCM <= 7.4 ~ "Toxicity", TRUE ~ "No toxicity") # Colon length <= 7.4 = Toxicity
data_lr$Outcome <- case_when(data_lr$Dose == "Toxicity" ~ 1,
                             TRUE ~ 0) # set "Tox" to 1
offset <- min(data_lr$Abundance[data_lr$Abundance > 0]) 
...
# Load data and normalize (relative abundance)
fn <- paste0(prefix, "ktrepka/GOStudy/GOPaper/GO16STables/TaxaTables/TaxaSummaryASVs.csv")
a <- read.csv(fn) %>% as.data.frame() 
rownames(a) <- a$X; a <- a %>% select(-X) # Wrangling
a <- a / colSums(a)*100 # Normalize to relative abundance (%)
a$Taxa <- rownames(a) # Re-add row names for melting
...
a$Sample_ID <- gsub("\\.", "-", a$Sample_ID)
a_plot <- inner_join(a, asvs, by = "ASV")
a_sum <- a_plot %>% group_by(Sample_ID) %>% dplyr::summarise(`Relative abundance` = sum(`Relative abundance`)) %>% as.data.frame()
df <- inner_join(a_sum, dates, by = "Sample_ID")
df <- df %>% filter(Treatment_Cycle %in% c("Baseline", "C1D1")) 
df <- df %>% group_by(Patient_ID) %>% dplyr::summarise(Abundance = mean(`Relative abundance`))
df <- df %>% select(Patient_ID, Abundance)
df$Patient_ID = as.numeric(df$Patient_ID)

# Tox data
fn <- paste0(prefix, "ktrepka/GOStudy/1_FIGURES/Compiled_Data/Predictions/GO_Tox.csv")
data_lr <- read.csv(fn) %>% as.data.frame() %>% select(-Abundance)
data_lr <- inner_join(data_lr, df, by = "Patient_ID")
data_lr$Outcome <- case_when(data_lr$Dose == "Toxicity" ~ 1,
                             TRUE ~ 0) # set "Tox" to 1
offset <- min(data_lr$Abundance[data_lr$Abundance > 0]) 
...
# Save
fn <- paste0(figdir, "ROC_GO_Species.pdf")
ggsave(filename = fn, plot = p, width = 3, height = 3)
```
